...
pub fn spawn_decoder(path: PathBuf, seek_ms: Option<u64>) -> Result<DecoderHandle, String> {
    ...
    thread::Builder::new()
        .name(format!("dec:{}", path.file_name().unwrap_or_default().to_string_lossy()))
        .spawn(move || {
...
fn push_decoded(
        ...
        ($b:expr, $norm:expr) => {{
            ...
            for i in 0..frames {
                ...
                loop {
                    ...
                    if producer.try_push(l).is_ok() && producer.try_push(r).is_ok() {
                        break;
                    }
                    thread::yield_now();
                }
    ...
    match buf {
        ...
        AudioBufferRef::F64(b) => {
            ...
            let chan1 = if n_channels > 1 { b.chan(1) } else { b.chan(0) };
            for i in 0..frames {
                if stop_flag.load(Ordering::Relaxed) { return written; }
                let l = chan0[i] as f32;
                let r = chan1[i] as f32;
                loop {
                    if stop_flag.load(Ordering::Relaxed) { return written; }
                    if producer.try_push(l).is_ok() && producer.try_push(r).is_ok() { break; }
                    thread::yield_now();
                }
                written += 1;
            }
